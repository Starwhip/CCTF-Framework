...
to reset
  ...
  if(output-file)[
    let SIM-PARAMS (word num-routers "," m0 "," numDefenders "," numDefenderScouts "," numAttackers "," numAttackerScouts "," attacker-move-chance "," vulnerability-spawn-delay "," router-vulnerability-chance "," router-takeover-chance "," vulnerability-detection-rate "," infection-detection-rate "," blue-team-win-cutoff)

    set filename open-file export-directory SIM-PARAMS ".csv"

    file-print "#- Config -#"

    file-print (word "Number of Routers," "Degree of Network," "Num Defenders," "Num Defender Scouts," "Num Attackers," "Num AttackerScouts," "Attacker Move Chance," "Vul Spawn Delay," "Router Vul Chance," "Router Comp Chance," "Vul Detection Rate," "Comp Detection Rate," "Num Entries")

    file-print SIM-PARAMS

    file-print "#- Prog Data -#"
...
to go
  ...
  if (ticks > blue-team-win-cutoff or (stop-if-attacker-win and attackers-won))[
    if output-file[
      file-print "#----- Prog End ------#"
      file-print "Attackers Compromised Center"
      file-print attackers-won
      file-close
    ]
    stop
  ]
  ...
  ask routers with [online = 0][
    if ticks >= next-enable-tick[
      if(routers-restart?)[ restart-router self ]
    ]
  ]
