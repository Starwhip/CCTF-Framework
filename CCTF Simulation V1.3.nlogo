extensions [rnd matrix table profiler]

;;;;;;;;;;;;;;;;;;;;;;;;
;;;    Turtle Data   ;;;
;;;;;;;;;;;;;;;;;;;;;;;;
globals [
  temp-node packet-target destroy-packet next-hop temp
  ...
  ;;Defender learning variables
  total-weight router-weight-backup current-stats last-stats current-learning-runs total-learning-runs
]
...
to setup
  ...
  ;;Learning variables
  set router-weight-backup table:make
  set current-stats table:make
  set last-stats table:make

  set current-learning-runs 0
  set total-learning-runs 0
...
to-report open-file [directory name ext]
  ...
  [
    ...
    let file (word basename "-" num "-" total-learning-runs ext)
    while [file-exists? file][
      set num num + 1
      set file (word basename "-" num "-" total-learning-runs ext)
    ]
...
to go-learn
  if (ticks >= blue-team-win-cutoff or (stop-if-attacker-win and attackers-won))[
    ...
    set current-learning-runs current-learning-runs + 1
    set total-learning-runs total-learning-runs + 1
    if current-learning-runs >= num-learning-runs [
      set current-learning-runs 0

      if (total-learning-runs > 100) [
        validate-router-weights
        if (total-learning-runs < 900) [ mod-router-weights ]
      ]
    ...
    if total-learning-runs > 1000 [ stop ]
    reset
  ]
...
to run-troops
  ...
  ask defenderscouts[
    ...
    if ticks >= next-action-tick[
      ...
      set starts lput get-agent-connected-router self starts
      set targets lput find-defender-scout-router-weights targets
    ]
  ]
...
to default-router-weights
  ask routers[
    set defender-select-weight 1.0
  ]

  backup-router-weights
end



to backup-router-weights
  ask routers[table:put router-weight-backup who defender-select-weight]
end

to restore-router-weights
  ask routers[set defender-select-weight table:get router-weight-backup who]
end

to-report score-defenders
  ;;TODO -- CHECK IF CURRENT AVERAGE STATS ARE BETTER OR WORSE AND SET ACCEPT TO TRUE OR FALSE

  let currentScore 0
  let newScore 0

  let metric-default 9999 * num-learning-runs
  set currentScore currentScore + (table:get-or-default last-stats "meanOffline" metric-default)
  ;;set currentScore currentScore + (table:get-or-default last-stats "maxOffline" metric-default)
  set currentScore currentScore + (table:get-or-default last-stats "meanCompromised" metric-default)
  ;;set currentScore currentScore + (table:get-or-default last-stats "maxCompromised" metric-default)
  set currentScore currentScore + (table:get-or-default last-stats "meanVulnerable" metric-default)
  ;;set currentScore currentScore + (table:get-or-default last-stats "maxVulnerable" metric-default)
  set currentScore currentScore + (table:get-or-default last-stats "metric2" metric-default)
  set currentScore currentScore + (table:get-or-default last-stats "metric3" metric-default)

  set newScore newScore + (table:get current-stats "meanOffline")
  ;;set newScore newScore + (table:get current-stats "maxOffline")
  set newScore newScore + (table:get current-stats "meanCompromised")
  ;;set newScore newScore + (table:get current-stats "maxCompromised")
  set newScore newScore + (table:get current-stats "meanVulnerable")
  ;;set newScore newScore + (table:get current-stats "maxVulnerable")
  set newScore newScore + (table:get current-stats "metric2")
  set newScore newScore + (table:get current-stats "metric3")

  let delta newScore / currentScore

  show (word "Stats for Run " total-learning-runs " | Delta: " (precision delta 2) " Score " (precision newScore 2))
  report delta
end

;;Pass a true or false to keep or reject this selection of routers
to validate-router-weights
  let accept true

  let finalscore score-defenders

  if (finalscore > 1.0)[
    set accept false
    ;;Random chance to still accept a bad run to escape a local minimum.
    if (random 100 <= 25) and (finalscore < 1.2) [ set accept true ]
  ]

  ifelse accept [
    show "   Accepted router weights"
    backup-router-weights
    set last-stats copy-table current-stats
    set current-stats table:make
  ] [
  ...
  ask routers [  set size defender-select-weight * 3]
end

to mod-router-weights
  let min-adjustment 0.1 + (0.85 * (1 - (e ^ (0.01 * (0 - (total-learning-runs - 100))))))
  let max-adjustment 1.9 - (0.85 * (1 - (e ^ (0.01 * (0 - (total-learning-runs - 100))))))
  let adjust 0
  ifelse random 2 = 0 [ set adjust min-adjustment ]  [ set adjust max-adjustment ]
  ask n-of (num-routers * 0.25) routers[
    set defender-select-weight defender-select-weight * adjust ;;(min-adjustment + random-float max-adjustment - min-adjustment)
  ]
end
...
to calculate-total-weight
  set total-weight 0
  ask routers[
    set total-weight total-weight + defender-select-weight
  ]
end

to-report find-defender-scout-router-weights
  let rand-weight random-float total-weight
  let accumulated-weight 0
  let selected nobody
  ask routers[
    set accumulated-weight accumulated-weight + defender-select-weight
    if (selected = nobody) and (accumulated-weight >= rand-weight) [ set selected self ]
  ]
...
to learn-metrics
  ...
  table:put current-stats "meanOffline" (table:get-or-default current-stats "meanOffline" 0 ) + (meanOffline / num-learning-runs * offline-weight)
  ;;table:put current-stats "maxOffline" (table:get-or-default current-stats "maxOffline" 0 ) + (maxOffline / num-learning-runs * offline-weight)
  table:put current-stats "meanVulnerable" (table:get-or-default current-stats "meanVulnerable" 0 ) + (meanVulnerable / num-learning-runs * vul-weight)
  ;;table:put current-stats "maxVulnerable" (table:get-or-default current-stats "maxVulnerable" 0 ) + (maxVulnerable / num-learning-runs * vul-weight)
  table:put current-stats "meanCompromised" (table:get-or-default current-stats "meanCompromised" 0 ) + (meanCompromised / num-learning-runs * comp-weight)
  ;;table:put current-stats "maxCompromised" (table:get-or-default current-stats "maxCompromised" 0 ) + (maxCompromised / num-learning-runs * comp-weight)

  table:put current-stats "metric2" (table:get-or-default current-stats "metric2" 0 ) + (metric2 * metric-2-weight)
  table:put current-stats "metric3" (table:get-or-default current-stats "metric3" 0 ) + (metric3 * metric-3-weight)
end
...
741
import-field
[[0 3 4 2 1] [1 14 8 5 0] [2 16 9 0] [3 0 6] [4 7 0] [5 1 18] [6 3 11] [7 12 4] [8 10 1] [9 2] [10 13 8] [11 6 23] [12 7] [13 10] [14 15 24 1] [15 14 17 19] [16 2 20] [17 22 15] [18 5] [19 15] [20 21 16] [21 20] [22 17] [23 11] [24 14]]
1
0
String
...
737
export-directory
D:/Netlogo/NetlogoSimulations/LearningRuns4/
1
0
String
